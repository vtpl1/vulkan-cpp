 ...
 *
 * @deprecated This is now no longer the preferred method of getting the handle
 *             of the surface. Instead of creating a class that extends
 *             {@link AWTVKCanvas}, you can create a generic canvas. With that
 *             canvas, use {@link AWTVK#create(Canvas, VkInstance)} with it, and
 *             it will return a surface handle. To draw, override
 *             {@link Canvas#paint(Graphics)} (need not call
 *             {@code super.paint(g)}). Other helpful utilities exist in
 *             {@link AWTVK}.
 *
 *             NOTE: Before the canvas can be passed into
 *             {@link AWTVK#create(Canvas, VkInstance)}, it must have a
 *             peer{{cn}}, and that can be forced by calling
 *             {@link Frame#pack()}.
 * @author Kai Burjack
 */
@Deprecated
public abstract class AWTVKCanvas extends Canvas {
	 ...
	 * {@link VkPhysicalDevice} in a command queue of the specified
	 * <code>queueFamiliy</code>.
	 *
	 * @param physicalDevice the Vulkan {@link VkPhysicalDevice}
	 * @param queueFamily    the command queue family
